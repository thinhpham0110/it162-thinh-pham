...
function loadContact($form, $feedback = '')
{
    ...
    if ($feedback == '') {
        $feedback = './includes/contact/feedback.php';
    }
    ...
    if (isset($_POST['g-recaptcha-response'])) :
        ...
        if ($resp->isSuccess()) :
            ...
            $fromAddress = ""; //default
            if (is_email($_POST['Email'])) { #Only use Email for return address if valid
...
{ #formats PHP POST data to text for email, feedback
    ...
    foreach ($_POST as $varName => $value) { #loop POST vars to create JS array on the current page - include email
        if (!in_array($varName, $aSkip) || $varName == 'Email') { #skip passover elements
            $strippedVarName = str_replace("_", " ", $varName); #remove underscores
            if (is_array($_POST[$varName])) { #checkboxes are arrays, and we need to loop through each checked item to insert
...
function email_handler($toEmail, $toName, $subject, $body, $fromEmail, $fromName, $website, $domain)
{
    ...
    if ($fromName == "") {
        $fromName = $website;
    } //default to website if name not provided
    ...
    if (@mail($toEmail, $subject, $body, implode(PHP_EOL, $headers))) { //only echo if debug is true
        if ($debug) {
            echo 'Email sent! ' . date("m/d/y, g:i A");
        }
    } else {
        if ($debug) {
